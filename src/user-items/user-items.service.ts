...
export class UserItemsService {
  ...
  async getInventory(characterId: string): Promise<UserItem[]> {
    if (!Types.ObjectId.isValid(characterId)) throw new BadRequestException('유효하지 않은 캐릭터 ID입니다.');
    return this.userItemModel.find({ characterId: new Types.ObjectId(characterId) }).exec();
  }

  async getInventoryItem(characterId: string, userItemId: string): Promise<UserItem> {
    if (!Types.ObjectId.isValid(characterId)) throw new BadRequestException('유효하지 않은 캐릭터 ID입니다.');
    if (!Types.ObjectId.isValid(userItemId)) throw new BadRequestException('유효하지 않은 아이템 ID입니다.');
    const item = await this.userItemModel.findOne({ 
  ...
  async buyItem(characterId: string, buyItemDto: BuyItemDto) {
    ...
    if (!itemMaster) throw new NotFoundException('존재하지 않는 시스템 아이템입니다.');
    ...
    if (itemMaster.type === 2) {
      let existingItem = await this.userItemModel.findOne({ 
        characterId: new Types.ObjectId(characterId), 
        itemId 
      });
      ...
      if (existingItem) {
        ...
        return { message: '아이템 구매 완료 (수량 증가)', character, item: existingItem };
      }
    }
  ...
  async sellItem(characterId: string, sellItemDto: SellItemDto) {
    ...
    if (!userItem) throw new NotFoundException('보유한 아이템을 찾을 수 없습니다.');
    if (userItem.quantity < quantity) throw new BadRequestException('보유 수량보다 많이 팔 수 없습니다.');
    if (userItem.isEquipped) throw new BadRequestException('장착 중인 아이템은 팔 수 없습니다. 장착 해제 후 판매하세요.');

    const itemMaster = await this.itemModel.findOne({ itemId: userItem.itemId });
    if (!itemMaster) throw new NotFoundException('시스템 아이템 정보가 누락되었습니다.');
    ...
    return { message: '아이템 판매 완료', sellPrice, currentGold: character.gold };
  }
  ...
  async useItem(characterId: string, useItemDto: UseItemDto) {
    ...
    if (itemMaster.type === 2) {
      if (character.mp < itemMaster.mp_spend) {
          throw new BadRequestException('마나가 부족하여 아이템을 사용할 수 없습니다.');
      }
    ...
    // 장비인 경우 (type: 0 공격, type: 1 방어) 장착/해제 토글
      ...
      if (userItem.isEquipped) {
        ...
        else if (itemMaster.type === 1) character.defence_item1_id = itemMaster.itemId; 
      } else {
        // 해제 시 캐릭터 스탯 감소
        character.attack_point -= itemMaster.attack;
        character.defence_point -= itemMaster.defence;
      ...
      await character.save();
      return { message: `아이템 장착 ${userItem.isEquipped ? '성공' : '해제'}`, character, userItem };
    }
  }
}
